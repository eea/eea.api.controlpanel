...
from eea.api.controlpanel.interfaces import IEEAVersionsFrontend


logger = logging.getLogger("eea.api.controlpanel")
...
def detectVersionChange(settings, event):
    ...
    if event.record.fieldName == 'version':
        ...
        transaction.get().note(
            "eea.api.controlpanel: updating FRONTEND_VERSION")
        transaction.commit()
